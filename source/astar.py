...
def a_star_with_path(get_neighbors, start_position, goal_test, heuristic):
    """ Generic A-Star algorithm.
    ...
    # (path length + h + heurtistic, Point, path length, Parent Node)
    pq = [(0, start_position, 0, [])]
    ...
    while len(pq) > 0:
        ...
        if goal_test(search_state[1]):
            # Found a path, return first node there 
            return search_state[3][0]
        
        else:
            for point in get_neighbors(search_state[1]):
                if point not in explored:
                    next_search_state = search_state[3] + [point]
                    heapq.heappush(pq, (search_state[2]+1 + heuristic(point), point, search_state[2]+1, next_search_state))
    return None 