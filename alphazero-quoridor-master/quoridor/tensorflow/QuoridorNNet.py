...
class QuoridorNNet():
    def __init__(self, game, args):
        ...
        with self.graph.as_default():
            self.input_boards = tf.placeholder(tf.float32, shape=[None, 4, self.board_x, self.board_y])    # s: batch_size x board_x x board_y
            self.dropout = tf.placeholder(tf.float32)
            self.isTraining = tf.placeholder(tf.bool, name="is_training")

            x_image = tf.reshape(self.input_boards, [-1, 4, self.board_x, self.board_y])                    # batch_size  x board_x x board_y x 1
